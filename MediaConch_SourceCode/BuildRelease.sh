...
function _obs () {
    ...
    cp prepare_source/archives/mediaconch_${Version_new}.tar.xz $OBS_package/mediaconch_${Version_new}.orig.tar.xz
    cp prepare_source/archives/mediaconch_${Version_new}.tar.gz $OBS_package

    # Create Debian packages and dsc
    deb_obs "$MC_tmp"/$OBS_package mediaconch_${Version_new}.orig.tar.xz
    ...
    update_PKGBUILD "$MC_tmp"/$OBS_package mediaconch_${Version_new}.orig.tar.xz PKGBUILD
...
function _linux () {
    ...
    if ! b.opt.has_flag? --only-images ; then
        ...
        if ! b.opt.has_flag? --jenkins ; then
            ...
            echo "python Handle_OBS_results.py $Filter $OBS_project MediaConch $Version_new \"$MCC_dir\" \"$MCD_dir\" \"$MCG_dir\"" >> "$WORKSPACE/STAGE"
...
function btask.BuildRelease.run () {
    ...
    if [ "$Target" = "linux" ] || [ "$Target" = "all" ] ; then
        find "$MCS_dir" -name 'mediaconch_*.tar.*' -mindepth 1 -delete
    fi
    if [ "$Target" = "mac" ] || [ "$Target" = "all" ] ; then
        find "$MCC_dir" "$MCD_dir" "$MCG_dir" -name 'MediaConch_*_GNU_FromSource.*' -mindepth 1 -delete
    fi
    if [ "$Target" = "windows" ] || [ "$Target" = "all" ] ; then
        find "$MCS_dir" -name 'mediaconch_*_AllInclusive.7z' -mindepth 1 -delete
    fi
