...
function load_options () {
    ...
    b.opt.add_opt --current "Current version of the project"
    b.opt.add_alias --current -c
    ...
    # Mandatory arguments
    b.opt.required_args --project --current
}
...
function run () {
    ...
    if b.opt.check_required_args; then
        ...
        Version_current=$(sanitize_arg $(b.opt.get_opt --current))

        Snapshot="no"
        if b.opt.has_flag? --snapshot; then
            Snapshot="yes"
            Version_new="${Version_current}.`date +%Y%m%d`"
